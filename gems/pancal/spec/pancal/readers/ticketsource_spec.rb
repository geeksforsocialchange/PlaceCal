...
# Ported from PlaceCal spec/jobs/calendar_importer/parsers/ticketsource_spec.rb.

RSpec.describe PanCal::Readers::Ticketsource do
  ...
  describe '.requires_api_token?' do
    it 'returns true (API-based reader)' do
  ...
  describe '.handles_url?' do
    it 'returns truthy for valid TicketSource venue URLs' do
      source = PanCal::Source.new(url: 'https://www.ticketsource.co.uk/fairfield-house')
      expect(described_class).to be_handles_url(source)
    end

    it 'returns falsy for non-TicketSource URLs' do
      source = PanCal::Source.new(url: 'https://www.example.com/events')
      expect(described_class).not_to be_handles_url(source)
    end
  end
  ...
  describe '#download_calendar' do
    ...
    context 'without API key' do
      let(:source) { PanCal::Source.new(url: url, token: nil) }

      it 'raises an error when API key is missing' do
        reader = described_class.new(source)
        expect { reader.download_calendar }.to raise_error(
          PanCal::InaccessibleFeed,
          /API key required/
        )
      end
    end
    ...
    context 'with API key' do
      ...
      let(:source) { PanCal::Source.new(url: url, token: api_key) }
  ...
  describe '#import_events_from' do
    let(:url) { 'https://www.ticketsource.co.uk/fairfield-house' }
    let(:source) { PanCal::Source.new(url: url, token: 'test_key') }
    ...
    it 'converts raw data to TicketsourceEvent objects' do
      events = reader.import_events_from([sample_event_data])

      expect(events).to be_an(Array)
      expect(events.length).to eq(1)
      expect(events.first).to be_a(PanCal::Events::TicketsourceEvent)
    end
    ...
    it 'filters out archived events' do
      ...
      events = reader.import_events_from([archived_event])
      expect(events).to be_empty
    end
    ...
    it 'filters out non-public events' do
      ...
      events = reader.import_events_from([private_event])
      expect(events).to be_empty
    end
    ...
    it 'returns empty array for non-array input' do
      expect(reader.import_events_from('invalid')).to eq([])
    end
  end
end
