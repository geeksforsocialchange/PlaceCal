...
# Ported from PlaceCal spec/jobs/calendar_importer/ld_json_parser_spec.rb.

RSpec.describe PanCal::Readers::LdJson do
  def check_source_has_events(url, cassette, expected_node_count, expected_event_count)
    VCR.use_cassette(cassette, allow_playback_repeats: true) do
      ...
      events = reader.import_events_from(records)
