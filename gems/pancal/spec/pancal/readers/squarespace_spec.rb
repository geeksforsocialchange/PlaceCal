...
# Ported from PlaceCal spec/jobs/calendar_importer/squarespace_parser_spec.rb.

RSpec.describe PanCal::Readers::Squarespace do
  ...
  describe '.handles_url?' do
    it 'returns true for squarespace.com URLs' do
      source = PanCal::Source.new(url: 'https://example.squarespace.com/events')
    ...
    it 'returns true for custom domain Squarespace sites with events' do
      ...
      source = PanCal::Source.new(url: 'https://www.example-squarespace.com/events')
      expect(described_class.handles_url?(source)).to be true
    end
    ...
    it 'returns false for Squarespace sites without events' do
      ...
      source = PanCal::Source.new(url: 'https://www.example-squarespace.com/blog')
      expect(described_class.handles_url?(source)).to be false
    end
  end
  ...
  describe '.squarespace_events?' do
    it 'returns true when JSON contains upcoming events' do
      json_response = { 'upcoming' => [{ 'id' => '123' }] }.to_json
      allow(PanCal::Readers::Base).to receive(:read_http_source).and_return(json_response)
    ...
    it 'returns false when JSON is invalid' do
      allow(PanCal::Readers::Base).to receive(:read_http_source).and_return('not json')
  ...
  describe '#import_events_from' do
    let(:source) { PanCal::Source.new(url: 'https://example.squarespace.com/events') }
    ...
    it 'converts Squarespace data to SquarespaceEvent objects' do
      events = reader.import_events_from(squarespace_data)

      expect(events.length).to eq(1)
      expect(events.first).to be_a(PanCal::Events::SquarespaceEvent)
    end
