...
# Ported from PlaceCal spec/jobs/calendar_importer/events/ics_event_spec.rb.
#
# ADAPTATION: the original spec asserted #online_event_id, which persisted an
# OnlineAddress record with a direct/indirect link_type. PanCal carries no
# persistence: #online_meeting_url returns the plain URL string (or nil) and
# the direct/indirect classification is the caller's job, driven by
# PanCal::Event::ONLINE_MEETING_DOMAINS. The direct/indirect intent of each
# original example is pinned here via that constant.

RSpec.describe PanCal::Events::IcsEvent do
  ...
  # Returns true when the caller would classify this URL as a direct online
  # meeting link (its host matches a known meeting/streaming domain).
  def direct_meeting_url?(url)
    host = URI.parse(url).host.to_s.downcase
    PanCal::Event::ONLINE_MEETING_DOMAINS.any? do |domain|
  ...
  describe '#online_meeting_url' do
    # Calendar custom properties for online meetings
    {
      ...
      context "when #{property} custom property is set" do
        ...
        it 'treats it as a direct online event' do
          expect(ics_event.online_meeting_url).to eq(url)
          expect(direct_meeting_url?(url)).to be true
        end
      end
    end
    ...
    [
      ...
      context "when #{description}" do
        ...
        it 'does not treat it as an online event' do
          expect(ics_event.online_meeting_url).to be_nil
        end
      end
    end
    ...
    %w[
      ...
      context "when location is generic URL #{url}" do
        ...
        it 'treats it as an indirect online event' do
          expect(ics_event.online_meeting_url).to eq(url)
          expect(direct_meeting_url?(url)).to be false
        end
      end
    end
