    ...
    class LdJson < Base
      ...
      class EventConsumer
        ...
        def consume_event(data)
          events << Events::LinkedDataEvent.new(data)
        end
        ...
        def consume(data)
          ...
          if data.is_a?(Hash) # rubocop:disable Style/GuardClause
            ...
            if dispatch_to
              ...
              PanCal.logger.debug { "unknown ld+json type '#{type}'" }
            end
            nil
          end
        end
        ...
        def validate_events
          events.select! { |ev| ev.valid? && ev.in_future? && ev.not_cancelled? }
        end
