  ...
  module Readers
    class Ics < Base
      ...
      def parse_remote_calendars(data)
        raise InvalidResponse.new('Source returned empty ICS data', code: :missing_data) if data.blank?
        ...
        raise InvalidResponse.new("Could not parse ICS response (#{e})", code: :invalid_ics)
      end
    end
  end
end
