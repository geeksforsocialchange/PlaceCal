...
class NeighbourhoodPolicyTest < ActiveSupport::TestCase
  ...
  def test_scope
    ...
    assert_equal(
      permitted_records(@neighbourhood_admin, Neighbourhood),
      @neighbourhood_admin.neighbourhoods
    )
    # Enforcing ordering is annoying, just compare the length, let the user test ensure that owned_neighbourhoods works
    assert_equal(
      permitted_records(@neighbourhood_region_admin, Neighbourhood).length,
      @neighbourhood_region_admin.owned_neighbourhoods.length
    )
    assert_equal(
      permitted_records(@root, Neighbourhood).length,
