...
class PartnerTest < ActiveSupport::TestCase
  setup do
    ...
    @new_partner =
      build(:partner, address: @new_address, accessed_by_user: @user)
  end
  ...
  test "it recycles addresses if it can" do
    ...
    assert @new_partner.address_id == other_partner.address_id,
           "should have same address ID"
  end
  ...
  test "validate name length" do
    ...
    assert_equal "must be at least 5 characters long",
                 error_map.options[:message]
  ...
  test "validate summary without description" do
    @new_partner.update(
      name: "Test Partner",
      summary: "This is a test partner used for testing :)",
      description: ""
    )
  ...
  test "validate description and summary" do
    ...
    assert @new_partner.errors.empty?,
           "Should be able to submit a summary and description"
  end
  ...
  test "validate summary length" do
    ...
    assert @new_partner.errors.key?(:summary),
  ...
  test "validate url" do
    ...
    assert_equal ["is invalid"],
                 @new_partner.errors[:url],
                 "Partner must have a valid url"
  ...
  test "validate twitter" do
    ...
    assert @new_partner.errors.key?(:twitter_handle),
           "Should be account name not full URL"
    @new_partner.update(twitter_handle: "asd£$%dsa")
    assert @new_partner.errors.key?(:twitter_handle),
           "Invalid Twitter account name saved"
  end

  test "validate facebook" do
    # Facebook must be valid
    @new_partner.update(facebook_link: "https://facebook.com/group-name")
    assert @new_partner.errors.key?(:facebook_link),
           "Should be page name not full URL"
    @new_partner.update(facebook_link: "Group-Name")
    assert @new_partner.errors.key?(:facebook_link),
           "invalid Facebook page name saved"
