...
class UserTest < ActiveSupport::TestCase
  ...
  test "owned neighbourhoods returns all descendants" do
    ...
    owned_length = 1 + (5 * 3)
    assert @neighbourhood_region_admin.owned_neighbourhoods.to_a.length ==
             owned_length

    # does it actually return both the districts and the wards?
    assert @neighbourhood_region_admin
             .owned_neighbourhoods
             .find_all { |u| u.unit == "county" }
             .length == 5
    assert @neighbourhood_region_admin
             .owned_neighbourhoods
             .find_all { |u| u.unit == "district" }
             .length == 5
    assert @neighbourhood_region_admin
             .owned_neighbourhoods
             .find_all { |u| u.unit == "ward" }
             .length == 5
  end
  ...
  test "can edit neighourhoods" do
    ...
    assert (
             @neighbourhood_region_admin.can_alter_neighbourhood_by_id? region.parent.id
           ) == false
    ...
    assert @neighbourhood_region_admin.can_alter_neighbourhood_by_id?(
             district.id
           )
