...
class Admin::ArticlesControllerTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @neighbourhood_admin =
      create(:neighbourhood_region_admin) do |admin|
  ...
  it_allows_access_to_new_for(
    %i[root editor partner_admin neighbourhood_admin]
  ) do
  ...
  it_allows_access_to_create_for(%i[root editor]) do
    ...
    assert_difference("Article.count") do
      post admin_articles_url, params: { article: article_hash }
    end
  ...
  # /update
  it_allows_access_to_update_for(
    %i[root editor partner_admin neighbourhood_admin]
  ) do
    patch admin_article_url(@article), params: { article: { name: "Foonly" } }
  ...
  # /destroy
  it_allows_access_to_destroy_for(
    %i[root editor partner_admin neighbourhood_admin]
  ) do
  ...
  it_denies_access_to_destroy_for(
    %i[partnerless_neighbourhood_admin citizen]
  ) do
    assert_difference("Article.count", 0) { delete admin_article_url(@article) }
  end
