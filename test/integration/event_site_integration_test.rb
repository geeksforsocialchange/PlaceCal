...
class EventForSiteScopeTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @partner =
      create(:partner, address: create(:address, neighbourhood: @neighbourhood))
...
class EventsBySiteTagTest < ActionDispatch::IntegrationTest
  test "filtering events by partner tag" do
    ...
    tag =
      Tag.create!(name: "Tag", slug: "tag", description: "A tag about a thing")
    ...
    address = Address.create!(street_address: "123 Street", postcode: "M15 5DD")
    assert address.neighbourhood == neighbourhood

    partner_with_tag =
      Partner.create!(name: "Partner with tag", address: address)

    # give same tag from site to partner
    partner_with_tag.tags << tag

    partner_without_tag =
      Partner.create!(name: "Partner without tag", address: address)
