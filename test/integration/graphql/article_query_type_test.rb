...
class ArticleIndexTest < ActionDispatch::IntegrationTest
  ...
  test "returns articles when invoked" do
    ...
    nodes.each do |gql_article|
      ...
      refute_nil article = Article.find_by(title: gql_article["name"]),
                 'Returned article that doesn\'t exist?'
      ...
      assert_field_equals gql_article,
                          "dateCreated",
                          value: article.created_at.iso8601
      assert_field_equals gql_article,
                          "dateUpdated",
                          value: article.updated_at.iso8601
      ...
      providers =
        assert_field gql_article,
                     "providers",
                     "Returned article has no providers"
