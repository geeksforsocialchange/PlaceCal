...
class GraphQLPartnerTest < ActionDispatch::IntegrationTest
  ...
  def check_address(data, address)
    ...
    assert_field_equals hood,
                        "abbreviatedName",
                        value: neighbourhood.abbreviated_name
    ...
    assert_field_equals hood,
                        "unitCodeValue",
                        value: neighbourhood.unit_code_value
  end
  ...
  def check_opening_hours(data, opening_hours)
    ...
    expected_day_of_week =
      expected_day["dayOfWeek"] =~ %r{/([^/]*)$} && Regexp.last_match(1)
  ...
  def check_areas_served(data, service_areas)
    ...
    assert_field_equals service_area,
                        "abbreviatedName",
                        value: wanted_area.abbreviated_name
    ...
    assert_field_equals service_area,
                        "unitCodeKey",
                        value: wanted_area.unit_code_key
    assert_field_equals service_area,
                        "unitCodeValue",
                        value: wanted_area.unit_code_value
  end
  ...
  def check_basic_fields(data, partner)
    ...
    assert_field_equals data,
                        ...
                        value: partner.accessibility_info
    assert_field_equals data, "url", value: partner.url
    assert_field_equals data,
                        "twitterUrl",
                        value: "https://twitter.com/#{partner.twitter_handle}"
  ...
  test "can view contact info when selected" do
    ...
    check_areas_served partner_data["areasServed"],
  ...
  test "can see published articles by partnetr" do
    ...
    assert article_data.length == 1,
           "Should only see the one article published by this partenr"
  end
