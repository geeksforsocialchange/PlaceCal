...
class ArticlesByPartnerTagTest < ActionDispatch::IntegrationTest
  test "returns published articles that are from a partner of a given tag, sorted by publish_date" do
    ...
    assert_equal 5,
                 articles.length,
                 "expected to only find articles that are published and tagged correctly"

    # newest to oldest
    expected_titles =
      [4, 3, 2, 1, 0].map { |index| live_tagged_articles[index].title }
