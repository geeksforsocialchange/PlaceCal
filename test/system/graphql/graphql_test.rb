...
module PlaceCalApi
  ...
  def configure(default_url_options)
    ...
    define_constant "ArticleConnectionQuery",
                    @client.parse(ARTICLE_CONNECTION_QUERY_STRING)

    define_constant "PartnersByTagQuery",
                    @client.parse(PARTNERS_BY_TAG_QUERY_STRING)
  end
...
class GraphQLTest < ApplicationSystemTestCase
  ...
  test "news article connection with image url" do
    ...
    url = article["image"]
    assert url =~ %r{\Ahttps://#{@server.host}:#{@server.port}/},
           "article image is not full URL"
  end
