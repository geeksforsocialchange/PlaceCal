...
class AdminPartnerTest < ApplicationSystemTestCase
  ...
  test "image preview on partner form" do
    ...
    attach_file(
      "partner_image",
      File.absolute_path(
        "./test/system/admin/damir-omerovic-UMaGtammiSI-unsplash.jpg"
      )
    )
    base64 =
      "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEASABIAAD/4gIcSUNDX1BST0ZJTEUAAQEAAAIMbGNtcwIQAABtbnRyUkdCIFhZWi"
    preview = find(:css, ".brand_image", wait: 15)
    assert preview["src"].starts_with?(base64),
           "The preview image src is not the expected value"
  end
  ...
  test "opening time picker on partner form" do
    ...
    within '[data-controller="opening-times"]' do
      ...
      click_button "Add"
      new_time =
        '{"@type":"OpeningHoursSpecification","dayOfWeek":"http://schema.org/Sunday","opens":"00:00:00","closes":"23:59:00"}'
      data =
        find('[data-opening-times-target="textarea"]', visible: :hidden).value
      # check it's in the list
      assert all(:css, ".list-group-item")[-1].text.starts_with?(
               "Sunday all day"
             )
