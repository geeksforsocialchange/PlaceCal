...
class EventsResolverTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @fake_eventbrite_event =
      FakeEventbriteEvent.new(
        id: "111111111111",
        name: {
          text: "A summary"
        },
        ...
        venue: nil,
        start: {
          local: @start_date.iso8601
        },
        end: {
          local: @end_date.iso8601
        },
        online_event: true
      )
    ...
    @fake_meetup_event =
      FakeMeetupEvent.new(
        ...
        description: {
          text: "<p>This is a meetup description!</p>"
        },
        venue: nil,
    ...
    @ics_event_data =
      CalendarImporter::Events::IcsEvent.new(
        @fake_ics_event,
        @start_date,
        @end_date
      )
  ...
  test "ics: can detect google meet url in custom properties" do
    ...
    @fake_ics_event[:custom_properties] = {
      "x_google_conference" => [meet_link]
    }
    ics_event_data =
      CalendarImporter::Events::IcsEvent.new(
        @fake_ics_event,
    ...
    assert_equal online_address.url,
                 @fake_ics_event["custom_properties"][
                   "x_google_conference"
                 ].first
  end
  ...
  test "ics: can detect jitsi link in description" do
    ...
    @fake_ics_event[
      :description
    ] = "Join us on jitsi: #{jitsi_link} words words words"
    ics_event_data =
      CalendarImporter::Events::IcsEvent.new(
        @fake_ics_event,
        @start_date,
  ...
  test "ics: can detect google meet link in description" do
    ...
    @fake_ics_event[
      :description
    ] = "Join us on meets: #{meet_link} words words words"
    ics_event_data =
      CalendarImporter::Events::IcsEvent.new(
        @fake_ics_event,
        @start_date,
  ...
  test "ics: can detect zoom link in description" do
    zoom_link =
      "https://us04web.zoom.us/j/78434510758?pwd=aILSsYSJRSb_uO87tFjulZuLAA0eXT.1"
    @fake_ics_event[
      :description
    ] = "join us on zoom: <p>#{zoom_link}<p> words words words"
    ics_event_data =
      CalendarImporter::Events::IcsEvent.new(
        @fake_ics_event,
        @start_date,
  ...
  test "eventbrite: can detect online event url" do
    eventbrite_link =
      "https://www.eventbrite.co.uk/e/some-random-event-woo-hoo-111111111111"
    @fake_eventbrite_event[:url] = eventbrite_link
    event_data =
      CalendarImporter::Events::EventbriteEvent.new(@fake_eventbrite_event)

    calendar = create(:calendar, strategy: "event")

    resolver =
      CalendarImporter::EventResolver.new(event_data, calendar, [], @start_date)
    resolver.determine_online_location
  ...
  test "meetup: can detect online event url" do
    meetup_link =
      "https://www.meetup.co.uk/e/some-random-event-woo-hoo-111111111111"
