...
namespace :db do
  ...
  task dump: :environment do
    ...
    with_config do |app, host, db, user|
      full_path =
        "#{backup_dir}/#{Time.now.strftime("%Y%m%d%H%M%S")}_#{db}.#{dump_sfx}"
      cmd =
        "pg_dump -F #{dump_fmt} -v -O -o -U '#{user}' -h '#{host}' -d '#{db}' -f '#{full_path}'"
    end
  ...
  namespace :dump do
    ...
    task table: :environment do
      ...
      if table_name.present?
        ...
        with_config do |app, host, db, user|
          full_path =
            "#{backup_dir}/#{Time.now.strftime("%Y%m%d%H%M%S")}_#{db}.#{table_name.parameterize.underscore}.#{dump_sfx}"
          cmd =
            "pg_dump -F #{dump_fmt} -v -O -o -U '#{user}' -h '#{host}' -d '#{db}' -t '#{table_name}' -f '#{full_path}'"
        end
      ...
      with_config do |app, host, db, user|
        ...
        when 1
          ...
          when "p"
            ...
            cmd =
              "pg_restore -F #{fmt} -v -c -C -U '#{user}' -h '#{host}' -d '#{db}' -f '#{file}'"
          end
        else
  ...
  task sync_prod_staging: :environment do
    prod_ssh_url =
      (ENV[DB_DUMP_SSH_URL] ? ENV[DB_DUMP_SSH_URL] : "root@placecal.org -p 666")

    ssh_url =
      if ENV[DB_DUMP_STAGING_SSH_URL]
  ...
  task dump_production_and_restore_other: :dump_production do
    if ENV["restore_on_local"]
      $stdout.puts "restore_on_local = #{ENV["restore_on_local"]}"
    end
    if ENV["restore_on_staging"]
      $stdout.puts "restore_on_staging = #{ENV["restore_on_staging"]}"
    end
