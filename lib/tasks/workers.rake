...
namespace :workers do
  ...
  task purge_all: :environment do
    ...
    Calendar
      .where(calendar_state: %i[in_worker in_queue])
      .each do |c|
  ...
  task sync_state: :environment do
    ...
    listed_active_calendars =
      Calendar.where(calendar_state: %i[in_worker in_queue]).map(&:id)

    pp [
         active: active_calendars,
         listed_active_calendars: listed_active_calendars
       ]

    # Prune the listed Calendars to find ones that are not running, but are listed as such
    ids_to_unset =
      listed_active_calendars.reject do |calendar_id|
        active_calendars.include?(calendar_id)
      end
