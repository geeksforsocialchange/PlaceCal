...
module CalendarImporter::Events
  class IcsEvent < Base
    class MissingTypeForURL < StandardError
    end
    ...
    def have_direct_url_to_stream?(link)
      ...
      domain =
        event_link_types
          .keys
          .find(proc { nil }) { |domain| link.include?(domain) }
    ...
    def event_link_types
      ...
      {
        "meet.jit.si" => {
          regex: %r{#{http}#{subdomain}meet.jit.si/#{suffix}},
          type: "direct"
        },
        "meet.google.com" => {
          regex: %r{#{http}#{subdomain}meet.google.com/#{suffix}},
          type: "direct"
        },
        "zoom.us" => {
          regex: %r{#{http}#{subdomain}zoom.us/j/#{suffix}},
          type: "direct"
        },
        "facebook.com" => {
          regex: %r{#{http}#{subdomain}facebook.com/events/#{suffix}},
          type: "indirect"
        }
      }
    end
