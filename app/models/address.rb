class Address < ApplicationRecord
  ...
  scope :find_by_street_or_postcode, ->(street, postcode) { where(street_address: street).or(where(postcode: postcode)) }
  ...
  class << self
    ...
    def search(components, postcode)
      ...
      @address ||= Address.where(postcode: postcode).first if postcode && postcode.length >= 6 #minimum length of a full postal code
