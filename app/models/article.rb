...
class Article < ApplicationRecord
  ...
  scope :with_partner_tag,
        lambda { |tag_id|
          joins(
            "left outer join article_partners on articles.id=article_partners.article_id"
          ).joins(
            "left outer join partner_tags on article_partners.partner_id = partner_tags.partner_id"
  ...
  scope :for_site,
        lambda { |site|
          ...
          if site_neighbourhood_ids.any?
            ...
            scope =
              scope
                ...
                .joins(
                  "LEFT OUTER JOIN partners ON article_partners.partner_id = partners.id"
                )
                .joins(
          ...
          if site_tag_ids.any?
            scope =
              scope.joins(
                " LEFT OUTER JOIN article_tags ON articles.id=article_tags.article_id"
              )
          ...
          scope =
            scope.where("(#{where_fragments.join(" OR ")})", *where_params)
