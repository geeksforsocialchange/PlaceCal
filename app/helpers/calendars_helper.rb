...
module CalendarsHelper
  def options_for_organiser
    org_opts =
      policy_scope(Partner).order(:name).collect { |opt| [opt.name, opt.id] }
  ...
  def options_for_location
    policy_scope(Partner).with_address.order(:name)
  end
  ...
  def calendar_import_sources
    ...
    parsers.each { |parser| yield parser::NAME, parser::DOMAINS }
  end
