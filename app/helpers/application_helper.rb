...
class StrongParametersFormBuilder < SimpleForm::FormBuilder
  def input(attribute_name, options = {}, &block)
    ...
    display_filter =
      self.options[:display_only]
        &.collect { |attr| attr.is_a?(Hash) ? attr.keys : attr }
        &.flatten
...
module ApplicationHelper
  ...
  def admin_nav_link(name, path, icon = false)
    content_tag :li, class: "nav-item" do
      ...
      if icon
        link_to "<i class='fa fa-#{icon} feather'></i> #{name}".html_safe,
                path,
                class: klass
      else
  ...
  def filtered_form_for(object, options = {}, &block)
    simple_form_for(
      object,
      options.merge(builder: StrongParametersFormBuilder),
      &block
    )
  end
