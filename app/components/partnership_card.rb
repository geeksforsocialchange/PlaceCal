...
class Components::PartnershipCard < Components::Base
  ...
  def view_template
    div(class: 'text-center w-full') do
      div(class: 'bg-home-green rounded-panel h-24 p-4') do
      ...
      link_to("#{@site_name} calendar", @link_to,
              class: 'flex items-center flex-col justify-center font-bold no-underline w-full rounded-pill bg-home-background-3 p-2 tp:h-[4.5rem] tp:p-4 transition duration-300 hover:bg-home-pink hover:text-home-background hover:outline-base-text')
    end
  end
end
