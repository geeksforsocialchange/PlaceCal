...
class Components::PullQuote < Components::Base
  ...
  def view_template(&)
    div(class: 'flex flex-col gap-6 text-center my-4') do
      blockquote(class: "flex flex-col items-center gap-8 tp:flex-row tp:gap-5 justify-evenly !my-0 #{'text-home-background' if @color_mode == 'dark'} [&_svg]:text-base-primary [&_svg]:size-10 [&_svg]:shrink-0") do
        raw(view_context.icon(:home_quote_open, size: nil))
        p(class: 'alt-title-small m-0 max-w-content text-pretty', &)
