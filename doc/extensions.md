...
| `nav_region_filter`     | boolean                                                                                                                    | `false`                         | Whether the nav includes the region control (`Components::RegionFilter`) as the last item before the theme CTA. Only renders when the site also has two or more Partnership tags.                                                                                                                                                                                                                      |
| `events_default_period` | `'future'`, `'week'` or `'day'`                                                                                            | none (core's density heuristic) | The events listing's period when the request names none. Set `'future'` for a listing designed as one flat list of everything upcoming; core otherwise picks day, week or future from how busy the calendar is.                                                                                                                                                                                        |
| `event_filter_style`    | `:date_picker`, `:day_strip` or `:day_strip_with_date_picker`                                                              | `:date_picker`                  | Which date control the events filter renders. `:day_strip_with_date_picker` renders the day strip plus the date picker's toggle button and hidden date field (dropping its own Today link, since the strip already has one), alongside the sort filter's toggle; both are wrapped in `.filters__toggles` for a theme to lay out as one row.                                                            |
